...
DISREGARD THIS - NOT RELEVANT BECAUSE PARTICIPANTS HEARD THE AUDIO ONCE BEFORE THE EVENT PHASE. Take out looks that come before the key syntactic frame.
```{r}
test_data <- test_data %>% mutate(frame_start = case_when(
  descriptor == "bamoule" ~ 1692,
  descriptor == "nuve" ~ 1761,
  descriptor == "daze" ~ 1586))

test_data <- test_data %>% 
  filter(time_since_trial_start >= audio_time_from_start + frame_start)
```

Add a column for when "la petite" ends.
```{r}
test_data <- test_data %>% mutate(frame_end = case_when(
  descriptor == "bamoule" ~ 1949,
  descriptor == "nuve" ~ 2098,
  descriptor == "daze" ~ 1800))
```

Add a column for when the novel word ends.
```{r}
test_data <- test_data %>% mutate(novel_word_end = case_when(
  descriptor == "bamoule" ~ 2692,
  descriptor == "nuve" ~ 2653,
  descriptor == "daze" ~ 2660))
```
...
DIREGARD THIS - Get mean times when "la petite" ends and the novel word ends.
```{r}
petite_end = mean(test_data$frame_end - test_data$frame_start)
novel_word_end = mean(test_data$novel_word_end - test_data$frame_start)
```
...
```{r}
test_data <- test_data %>% 
  mutate(time_since_disambig = time_since_trial_start - (audio_time_from_start + frame_start))
```
...
test_data_timecourse <- test_data %>% 
  mutate(time_since_trial_start_ms = time_since_trial_start/10) #%>%

# PUT THIS LATER
  mutate(timebin = plyr::round_any(time_since_trial_start, 50)) %>%
  filter(time_since_event_start_ms <= 10000)
# NOTE THAT HERE WE CONVERT TO MS

test_data_timecourse = test_data_timecourse %>% group_by(participant_id, trial_no) %>%
  mutate(time_since_event_start = time_since_trial_start_ms - min(time_since_trial_start_ms))

test_data_timecourse = test_data_timecourse %>% mutate(timebin = plyr::round_any(time_since_trial_start_ms, 50))#%>%
  filter(time_since_event_start_ms <= 10000) # FIX THIS
...
prop_action_timecourse_sum <- test_data_timecourse %>% 
  ...
  summarise(timebin_prop_action_look = mean(look_action_video))
```

Make the plot.
```{r}
ggplot(prop_action_timecourse_sum) +
  aes(x = timebin_ms_from_start, y = timebin_prop_action_look, color = condition) +
  geom_point(size = .8) +
  geom_smooth() +
  #geom_line() +
  ggtitle("Proportion of looks to action video on test trials") +
  xlab("Time since start of event phase (ms)") +
  ylab("Mean proportion of looks") +
  scale_color_manual(name="Condition", values = c(wes_palette("Royal1")[2], "dodgerblue3")) +
