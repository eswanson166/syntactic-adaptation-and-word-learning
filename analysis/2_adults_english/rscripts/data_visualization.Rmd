...
```{r}
girls_end = mean(test_data$frame_end - test_data$frame_start)
...
Compare to kid study:
```{r}
test_action_sum_kids = action_look_sum
test_action_sum_kids = rename(test_action_sum_kids, m_prop_looks_action = avg_action_look)
# TODO: FIX THIS LATER
```


```{r}
test_action_sum_fr$study <- "French - adults"
test_action_sum$study <- "English - adults"
test_action_sum_kids$study <- "English - children"

test_action_combo <- rbind(test_action_sum_fr, test_action_sum, test_action_sum_kids)
test_action_combo$condition <- factor(test_action_combo$condition, levels = c("baseline", "noun", "verb"))

test_action_combo$study <- factor(test_action_combo$study, levels = c("French - adults", "English - adults", "English - children"))
```
...
ggplot(test_action_combo) +
  ...
  geom_errorbar(data = test_action_combo, 
                aes(ymin = YMin, ymax = YMax), width = .25)
ggsave(file="../graphs/prop_looks_compare_studies_v.pdf",width=7.5,height=4)
```



Combine just adult studies:

```{r}
test_action_combo_adult = test_action_combo %>% filter(study != "English - children")
test_action_combo_adult$study = droplevels(test_action_combo_adult$study)
levels(test_action_combo_adult$study) = c("French - adults", "English - adults")
```


```{r}
ggplot(test_action_combo_adult) +
  aes(x = condition, y = m_prop_looks_action, fill = condition) +
  geom_bar(stat = "identity") +
  facet_wrap(study ~ .) +
  scale_fill_manual(name = "Condition",values = c(wes_palette("Royal1")[1], wes_palette("Royal1")[2], "dodgerblue3")) +
  ...
  geom_errorbar(data = test_action_combo, 
                aes(ymin = YMin, ymax = YMax), width = .25)
ggsave(file="../graphs/prop_looks_compare_studies_adults.pdf",width=7.5,height=4)
```